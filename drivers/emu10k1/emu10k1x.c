...
/* open callback */
int snd_emu10k1x_playback_open(struct snd_pcm_substream *substream)
{
...
/* hw_params callback */
int snd_emu10k1x_pcm_hw_params(struct snd_pcm_substream *substream,
...
/* prepare callback */
int snd_emu10k1x_pcm_prepare(struct snd_pcm_substream *substream)
{
...
/* operators */
const struct snd_pcm_ops snd_emu10k1x_playback_ops = {
...
unsigned char emu10k1x_mpu401_read (struct snd_card *card, unsigned int idx)
{
  struct emu10k1x *emu = card->private_data;
  return (unsigned char)snd_emu10k1x_ptr_read(emu, MUDATA + idx, 0);
}

void emu10k1x_mpu401_write (struct snd_card *card, unsigned int idx, unsigned char data)
{
  struct emu10k1x *emu = card->private_data;
  snd_emu10k1x_ptr_write(emu, MUDATA + idx, 0, data);
}
...
u16 emu10k1x_ac97_read (struct snd_card *card, u8 reg)
{
  u16 val;
  struct emu10k1x *emu = card->private_data;
  spin_lock_irqsave(&emu->emu_lock, flags);
...
void emu10k1x_ac97_write (struct snd_card *card, u8 reg, u16 val)
{
  struct emu10k1x *emu = card->private_data;
  spin_lock_irqsave(&emu->emu_lock, flags);
  linux_outb(reg, emu->port + AC97ADDRESS);
...
static void emu10k1x_ac97_init (struct snd_card *card)
{
  int i;
