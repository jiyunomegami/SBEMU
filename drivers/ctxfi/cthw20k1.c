...
static int uaa_to_xfi(struct pci_dev *pci)
{
	...
	unsigned long __iomem mem_base;
	int i;
...
extern unsigned int xfi_int_cnt;
...
static int hw_card_start(struct hw *hw)
{
	...
	if (hw->irq < 0) {
        ...
        //printk("hw_card_init 3 io_base: %X  irq: %u\n", hw->io_base, hw->irq);
