...
static void pci_read_bridge_io(struct pci_bus *child)
{
	...
	if (base <= limit) {
		...
		pci_info(dev, "  bridge window %X - %X\n", res->start, res->end);
	}
}
	...
	if (dev->transparent) {
		pci_bus_for_each_resource(child->parent, res) {
			if (res && res->flags) {
				...
				pci_info(dev, "  bridge window %X - %X (subtractive decode)\n", res->start, res->end);
			}
		}
	}
}
...
void pci_dump_device (struct pci_dev *dev)
{
  int sy, sx;
  for (sy = 0; sy < 16; sy++) {
    DBG_Logi("%8.8X ", sy*16);
    for (sx = 0; sx < 16; sx++) {
      u8 val;
      pci_read_config_byte(dev, sy*16+sx, &val);
      DBG_Logi(" %2.2X", val);
    }
    DBG_Logi("\n");
  }
 ...
 */
void __weak xxxprobe_pcibios_fixup_bus(struct pci_bus *bus)
{
       /* nothing to do, expected to be removed in the future */
}
// in common.c
extern void pcibios_fixup_bus(struct pci_bus *b);
