...
//XXX static DECLARE_DELAYED_WORK(pci_pme_work, pci_pme_list_scan);
				  ...
				  const char **endptr)
{
	...
	*endptr = strchrnul(path, ';');
	...
	//wpath = kmemdup_nul(path, *endptr - path, GFP_ATOMIC);
        wpath = kmalloc(*endptr - path, GFP_ATOMIC);
	if (!wpath)
		return -ENOMEM;
        memcpy(wpath, path, *endptr - path);
 ...
 * this if necessary.
 */
void __weak linux_pcibios_set_master(struct pci_dev *dev)
{
	u8 lat;
...
static struct attribute *pci_dev_reset_method_attrs[] = {
  //&dev_attr_reset_method.attr, // XXX
	NULL,
};
...
//{
//  printk("UNIMPLEMENTED pci_disable_bridge_window\n");
//}
void pci_allocate_vc_save_buffers(struct pci_dev *dev)
{
  //printk("UNIMPLEMENTED pci_allocate_vc_save_buffers\n");
}
void pci_restore_vc_state(struct pci_dev *dev)
{
  printk("UNIMPLEMENTED pci_restore_vc_state\n");
}
int pci_save_vc_state(struct pci_dev *dev)
{
  printk("UNIMPLEMENTED pci_save_vc_state\n");
  return 0;
}
