...
static void snd_trident_ac97_init (struct snd_trident *trident)
{
  ...
  for (i = 0; ac97_initial_values[i].port != 0xffff; i++) {
    snd_trident_codec_write(&ac97, ac97_initial_values[i].port, ac97_initial_values[i].value);
  }
}

u16 snd_trident_ac97_read (struct snd_card *card, u16 reg)
{
  struct snd_ac97 ac97;
  ac97.private_data = card->private_data;
  ac97.num = 0;
  return snd_trident_codec_read(&ac97, reg);
}

void snd_trident_ac97_write (struct snd_card *card, u16 reg, u16 val)
{
  struct snd_ac97 ac97;
  ac97.private_data = card->private_data;
  ac97.num = 0;
  snd_trident_codec_write(&ac97, reg, val);
}
...
int snd_trident_hw_params(struct snd_pcm_substream *substream,
                          struct snd_pcm_hw_params *hw_params)
{
	int err;
...
int snd_trident_playback_prepare(struct snd_pcm_substream *substream)
{
	struct snd_trident *trident = snd_pcm_substream_chip(substream);
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_trident_voice *voice = runtime->private_data;
...
int snd_trident_trigger(struct snd_pcm_substream *substream,
                        int cmd)
...
snd_pcm_uframes_t snd_trident_playback_pointer(struct snd_pcm_substream *substream)
{
	struct snd_trident *trident = snd_pcm_substream_chip(substream);
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_trident_voice *voice = runtime->private_data;
...
int snd_trident_playback_open(struct snd_pcm_substream *substream)
{
	struct snd_trident *trident = snd_pcm_substream_chip(substream);
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_trident_voice *voice;
...
int snd_trident_synth_open(struct snd_pcm_substream *substream)
{
	struct snd_trident *trident = snd_pcm_substream_chip(substream);
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_trident_voice *voice;
	voice = snd_trident_alloc_voice(trident, SNDRV_TRIDENT_VOICE_TYPE_SYNTH, 0, 0);
	if (voice == NULL)
		return -EAGAIN;
	snd_trident_pcm_mixer_build(trident, voice, substream);
	voice->substream = substream;
...
const struct snd_pcm_ops snd_trident_playback_ops = {
