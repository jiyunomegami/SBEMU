...
***/
// in oxygen.c
extern const struct pci_device_id oxygen_ids[];
extern int get_oxygen_model(struct oxygen *chip,
                            const struct pci_device_id *id);
int snd_oxygen_probe (struct snd_card *card, struct pci_dev *pci, int probe_only)
{
	struct oxygen *chip;
	...
	pci_id = oxygen_search_pci_id(chip, oxygen_ids);
	if (!pci_id)
		return -ENODEV;

	oxygen_restore_eeprom(chip, pci_id);
	//err = get_model(chip, pci_id);
        err = get_oxygen_model(chip, pci_id);
	if (err < 0)
		return err;
